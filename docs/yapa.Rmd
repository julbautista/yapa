...
The number of respondents in each poll $y_p$ is drawn from the total respondents in that poll $n_p$ at a rate specific to the candidate and the state $\theta_{s, c}$.  The rate parameters is distributed with a candidate-state specific center $\alpha_{c, s}$ and state specific scale $t_s$.  The sum of proportions for each candidate must equal one.
